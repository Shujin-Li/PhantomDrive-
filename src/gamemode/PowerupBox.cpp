...
bool PowerupBox::tryCollect(const QVector2D& playerPosition, const QString& playerId, float collectRadius)
{
    if (!m_isActive) {
        return false;
    }

    const float radius = collectRadius > 0.0f ? collectRadius : m_radius;
