...
void ArcadeHUD::setupUI()
{
    ...
    // ========================================================================
    // COUNTDOWN overlay (absolute, centred over the dial)
    // ========================================================================
    m_countdownLabel = new QLabel("", this);
    m_countdownLabel->setAlignment(Qt::AlignCenter);
    m_countdownLabel->setStyleSheet(
        "QLabel{color:#FFE000;font-size:72px;font-weight:bold;"
        "background:rgba(0,0,0,130);border-radius:10px;"
        "font-family:'Segoe UI',sans-serif;}");
    m_countdownLabel->setVisible(false);
    m_countdownLabel->raise();
}

void ArcadeHUD::repositionCountdown()
{
    if (m_countdownLabel && m_speedo) {
        const QRect sr = m_speedo->geometry();
        m_countdownLabel->setGeometry(sr.x(), sr.y() + sr.height() / 2 - 50,
                                      sr.width(), 100);
    }
}
...
void ArcadeHUD::updateLap(int lapsCompleted, int totalLaps)
{
    ...
    if (m_lapLabel)
        m_lapLabel->setText(QString("%1 / %2").arg(lapsCompleted).arg(totalLaps));
}

void ArcadeHUD::updateRouteProgress(int, int, const QString& nextTarget)
{
    if (m_lapLabel) m_lapLabel->setText(nextTarget);
}
...
void ArcadeHUD::onCountdownTick()
{
    ...
    if (m_countdownValue > 0) {
        if (m_countdownLabel) m_countdownLabel->setText(QString::number(m_countdownValue));
    } else {
...
void ArcadeHUD::showGo()
{
    if (m_countdownLabel) {
        ...
        m_countdownLabel->setStyleSheet(
            "QLabel{color:#00FFA0;font-size:72px;font-weight:bold;"
            "background:rgba(0,0,0,130);border-radius:10px;"
            "font-family:'Segoe UI',sans-serif;}");
    }
...
void ArcadeHUD::reset()
{
    ...
    if (m_lapLabel)        m_lapLabel->setText("1 / 3");
...
//  Stubs
// ---------------------------------------------------------------------------
void ArcadeHUD::showFloatingMessage(const QString&, const QString&) {}
