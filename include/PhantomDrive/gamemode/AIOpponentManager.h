...
public:
    ...
    // ==================== 赛道设置 ====================
    virtual void setTrackManager(TrackManager* trackManager);
    ...
    virtual void setRaceTotalLaps(int laps);
    virtual int getRaceTotalLaps() const;

    // ==================== Race progress / ranking ====================
    virtual void setPlayerRaceProgress(int lap,
                                       int checkpointIndex,
                                       qreal progressPercent,
                                       bool finished = false,
                                       qreal totalTime = 0.0);
    virtual int getPlayerRacePosition() const;
    virtual QStringList getRaceOrder() const;
    virtual QJsonObject raceResultsToJson() const;
...
private:
    ...
    void refreshRaceProgressFromOpponents();
    void updateRaceRankings();
    void applyBoundaryRecovery(AIOpponent* opponent);
    void applySimpleAvoidance();
    qreal raceSortValue(const RaceEntry& entry) const;
    bool areAllOpponentsFinished() const;
    ...
    RaceEntry m_playerRace;
    QMap<QString, RaceEntry> m_aiRace;
    QStringList m_raceOrder;
    QStringList m_finishOrder;
    int m_totalLaps;
    bool m_allOpponentsFinishedEmitted;
};
