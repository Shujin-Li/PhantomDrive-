...
- `CMakeLists.txt`
  ...
  - 移除 `GameTopBar.h/cpp`（已整合入 `.ui` 布局）。

- `assets/mainwindow.ui`
  - `hudLayout` 新增 `label_ModeTitle`（居中模式标题）和 `btn_ExitGame_Top` / `btn_FinishDrive_Top` 按钮。
  - 三个按钮统一固定高度 52px。

- `include/PhantomDrive/UI/mainwindow.h`
  - 移除 `GameTopBar` 相关 include 和 `m_gameTopBar` 成员变量。

- `src/UI/mainwindow.cpp`
  - 移除所有 `m_gameTopBar` 创建/连接/show/move 代码。
  - `setupUi()` 后统一应用三个按钮的 cyberpunk 样式。
  - `startDrivingSession()` 中更新 `label_ModeTitle` 文字和颜色。
  - `startCustomTrackSession()` 中同样更新 `label_ModeTitle`。
  - `setGameHeaderVisible()` 新增对 `label_ModeTitle`、`btn_FinishDrive_Top`、`btn_ExitGame_Top` 的显示控制。
  - 右侧 HUD 定位改为固定 `y = hudBarHeight(42) + 8`。
