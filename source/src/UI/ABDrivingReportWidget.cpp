...
QString scoreViolationTypeName(ViolationType type)
{
    switch (type) {
    case ViolationType::None:
        return QStringLiteral("None");
    case ViolationType::RedLight:
        return QStringLiteral("RedLight");
    case ViolationType::SpeedOverLimit:
        return QStringLiteral("SpeedOverLimit");
    case ViolationType::PedestrianCollision:
        return QStringLiteral("PedestrianCollision");
    case ViolationType::WrongWay:
        return QStringLiteral("WrongWay");
...
void ABDrivingReportWidget::setReport(const ScoreReport& report)
{
    setReportSummary(summaryFromScoreReport(report));
}

void ABDrivingReportWidget::setCurrentReport(const ScoreReport& report)
{
    setReport(report);
}

void ABDrivingReportWidget::setCoachReportMarkdown(const QString& markdown)
{
    updateCoachAdvice(markdown);
}
...
void ABDrivingReportWidget::loadHistoryReports(const QList<ScoreReport>& reports)
{
    QList<ReportSummary> summaries;
    summaries.reserve(reports.size());
    for (const ScoreReport& report : reports) {
        summaries.append(summaryFromScoreReport(report));
    }
    setHistoryReports(summaries);
}
...
ReportSummary ABDrivingReportWidget::summaryFromScoreReport(const ScoreReport& report)
{
    ReportSummary summary;
    summary.totalScore = qBound(0, qRound(report.totalScore), 100);
    summary.grade = report.grade;
    summary.averageSpeed = report.metrics.averageSpeed;
    summary.maxSpeed = report.metrics.maxSpeed;
    summary.coachAdvice = report.summary;

    QStringList coachLines;
    if (!summary.coachAdvice.trimmed().isEmpty()) {
        coachLines << summary.coachAdvice.trimmed();
    }
    for (const CoachAdvice& advice : report.coachAdvices) {
        coachLines << QStringLiteral("[%1/%2] %3")
    ...
    summary.coachAdvice = coachLines.isEmpty()
        ? QStringLiteral("No coach advice provided.")
        : coachLines.join(QStringLiteral("\n\n"));

    summary.violations.reserve(report.violations.size());
    for (const ViolationEvent& violation : report.violations) {
        ViolationItem item;
        item.type = scoreViolationTypeName(violation.type);
        item.description = violation.description;
        item.speed = violation.speedAtViolation;
        item.limit = violation.speedLimit;
        item.penalty = violation.penaltyPoints;
        item.timestamp = violation.timestamp;
        summary.violations.append(item);
    }
