...
void InteractiveFeedback::setupUI()
{
    ...
    m_countdownLabel->setWindowFlags(Qt::Tool | Qt::FramelessWindowHint | Qt::WindowStaysOnTopHint);
    m_countdownLabel->setAttribute(Qt::WA_TranslucentBackground);
    m_countdownLabel->setAttribute(Qt::WA_TransparentForMouseEvents);
    m_countdownLabel->raise();
    m_countdownLabel->setStyleSheet(R"(
        QLabel {
            background: rgba(6, 12, 28, 240);
            color: #FFE000;
...
void InteractiveFeedback::showCountdown(int seconds)
{
    ...
    const QRect gvRect = m_gameView->rect();
    const QPoint gvGlobal = m_gameView->mapToGlobal(QPoint(0, 0));
    const int cx = gvGlobal.x() + (gvRect.width() - cdW) / 2;
    const int cy = gvGlobal.y() + (gvRect.height() - cdH) / 2 - 80;
    m_countdownLabel->resize(cdW, cdH);
    m_countdownLabel->move(cx, cy);
    m_countdownLabel->setStyleSheet(R"(
        QLabel {
            background: rgba(6, 12, 28, 240);
            color: #FFE000;
    ...
    m_countdownStep = seconds;
    m_countdownLabel->setText(QString::number(m_countdownStep));
    PhantomDrive::SoundManager::instance().play(PhantomDrive::SoundEffect::CountdownThree);

    auto tick = [this, interval, cx, cy]() {
        m_countdownStep--;
        if (m_countdownStep > 1) {
            m_countdownLabel->setText(QString::number(m_countdownStep));
            m_countdownLabel->move(cx, cy);
            m_countdownLabel->raise();
            PhantomDrive::SoundManager::instance().play(PhantomDrive::SoundEffect::CountdownTwo);
            m_countdownTimer->setSingleShot(true);
            m_countdownTimer->start(interval);
        } else if (m_countdownStep == 1) {
            m_countdownLabel->setText(QString::number(m_countdownStep));
            m_countdownLabel->move(cx, cy);
            m_countdownLabel->raise();
            PhantomDrive::SoundManager::instance().play(PhantomDrive::SoundEffect::CountdownOne);
            m_countdownTimer->setSingleShot(true);
            m_countdownTimer->start(interval);
        } else {
            m_countdownTimer->stop();
            ...
            m_countdownLabel->move(cx, cy);
            m_countdownLabel->raise();
            PhantomDrive::SoundManager::instance().play(PhantomDrive::SoundEffect::CountdownGo);
    ...
    connect(m_countdownTimer, &QTimer::timeout, this, tick);
    m_countdownTimer->setSingleShot(true);
    m_countdownTimer->start(interval);
}
