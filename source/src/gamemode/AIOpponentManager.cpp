...
AIOpponent* AIOpponentManager::createOpponent(const QString& id, AIStyle style)
{
    ...
    if (m_trackManager) {
        opponent->initializePhysics(m_trackManager);
    }
...
void AIOpponentManager::setTrackManager(TrackManager* trackManager)
{
    m_trackManager = trackManager;
    for (AIOpponent* opponent : m_opponents) {
        if (auto* simple = qobject_cast<SimpleAIOpponent*>(opponent)) {
            simple->initializePhysics(trackManager);
        }
    }
}
...
void AIOpponentManager::applySimpleAvoidance()
{
    ...
    for (int i = 0; i < ids.size(); ++i) {
        ...
        for (int j = i + 1; j < ids.size(); ++j) {
            ...
            const QVector2D aNewPos = a->getPosition() - direction * overlap;
            const QVector2D bNewPos = b->getPosition() + direction * overlap;

            if (auto* simpleA = qobject_cast<SimpleAIOpponent*>(a)) {
                if (simpleA->isPositionFree(aNewPos)) {
            ...
            if (auto* simpleB = qobject_cast<SimpleAIOpponent*>(b)) {
                if (simpleB->isPositionFree(bNewPos)) {
